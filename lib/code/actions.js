...
export async function startInteractiveMode(id) {
  ...
  try {
    const agent = workspace.codingAgent || 'claude-code';
...
export async function createTerminalSession(id, type = 'shell') {
  ...
  try {
    ...
    const agentCmd = agentCmdMap[workspace.codingAgent || 'claude-code'] || 'claude --dangerously-skip-permissions';
...
export async function getWorkspaceDiffStats(id, authenticatedUser) {
  ...
  try {
    ...
    const diffBranch = workspace.featureBranch || workspace.branch || 'main';
    const opts = { cwd: repoPath, encoding: 'utf8', timeout: 5000 };

    // Tracked changes (committed + staged + unstaged) vs origin feature branch
    const numstat = execSync(
      `git -c safe.directory='*' diff --numstat origin/${diffBranch} 2>/dev/null`,
      opts
    );
    ...
    for (const file of untracked.split('\n')) {
      if (!file) continue;
      try {
        const count = execSync(`wc -l < "${file}"`, opts);
        insertions += parseInt(count.trim(), 10);
      } catch {}
    }
...
export async function getWorkspaceDiffFull(id, authenticatedUser) {
  ...
  try {
    ...
    try {
      diff = execSync(
        `git -c safe.directory='*' diff origin/${diffBranch} 2>/dev/null`,
        opts
      );
    } catch {}
 ...
 * @param {string} id - Workspace ID
 * @param {string} command - 'commit' | 'push' | 'create-pr' | 'rebase' | 'resolve-conflicts'
...
export async function runWorkspaceCommand(id, command) {
  ...
  try {
    ...
    const prompts = {
      'commit': 'All changes have been staged with git add -A. Review the staged diff with `git diff --cached`. Write a clear conventional commit message and run `git commit` with that message. Do not modify any files.',
      'push': 'All changes have been staged with git add -A. Review the staged diff with `git diff --cached`. Write a clear conventional commit message and run `git commit` with that message. Do not modify any files.',
